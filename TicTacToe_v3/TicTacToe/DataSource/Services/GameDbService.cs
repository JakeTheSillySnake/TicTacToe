...
(AppDbContext dbContext, IGameService gameService) : IGameDbService
{
    ...
    public async Task<int> ClearGame(string uuid)
    {
    ...
    public async Task<List<CurrentGameDTO>> GetFinishedGames(string userUuuid)
    {
        return await _dbContext.Games
            .Where(game => (game.PlayerO == userUuuid || game.PlayerX == userUuuid)
                && game.State >= (int)GameStates.PLAYERX_WIN)
            .ToListAsync();
    }
    public async Task<List<UserWinRatioDTO>> GetUserWinRatios(int n)
    {
        string query = @"
            SELECT ""Uuid"", SUM(""Won"") AS ""Wins"",
            (CASE
                WHEN ""PlayedBefore"" = 0 THEN 0 ELSE COUNT(*)
            END) AS ""Total"",
            (CASE 
                WHEN COUNT(*) = 0 THEN 0 ELSE 
                    CAST(CAST(SUM(""Won"") AS DECIMAL(10,4)) / 
                        COUNT(*) AS DECIMAL(10,4)) 
            END) AS ""WinRatio""
            FROM (SELECT u.""Uuid"",
                    (CASE
                        WHEN g.""State"" is NULL THEN 0 ELSE 1
                    END) as ""PlayedBefore"",
                    (CASE
                        WHEN(g.""PlayerX"" = u.""Uuid"" AND g.""State"" = {0})
                        OR (g.""PlayerO"" = u.""Uuid"" AND g.""State"" = {1}) THEN 1 ELSE 0
                    END) as ""Won""
                    FROM ""Users"" u
                    LEFT JOIN ""Games"" g 
                        on u.""Uuid"" = g.""PlayerX"" OR u.""Uuid"" = g.""PlayerO""
                    WHERE g.""State"" >= {0} OR g.""State"" IS NULL
            )
            GROUP BY ""Uuid"",""PlayedBefore""
            ORDER BY ""WinRatio"" DESC, ""Total"" DESC
            LIMIT {2}
        ";

        return await _dbContext.UserWinRatios
            .FromSqlRaw(query, (int)GameStates.PLAYERX_WIN, (int)GameStates.PLAYERO_WIN, n)
            .ToListAsync();
    }
